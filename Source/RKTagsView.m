...
- (void)commonSetup {
    ...
    self.moreButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 100, 30)];
...
- (void)setOneLineMode:(BOOL)oneLineMode {
    if (_oneLineMode != oneLineMode) {
        _oneLineMode = oneLineMode;
        [self setNeedsLayout];
    }
}
...
- (void)layoutSubviews {
    ...
    NSMutableArray<UIButton *> *processedButtons = @[].mutableCopy;
    for (UIButton *button in self.mutableTagButtons) {
        CGRect buttonFrame = [self originalFrameForView:button];
        CGFloat spaceUsed = CGRectGetMaxX(previousButtonFrame) + self.interitemSpacing + buttonFrame.size.width;

        if (_scrollsHorizontally || spaceUsed <= contentWidth) {
            ...
            [processedButtons addObject:button];

            button.hidden = NO;
        } else {
            if (self.oneLineMode) {
                [self.moreButton sizeToFit];
                spaceUsed -= button.frame.size.width;
                self.moreButton.frame = CGRectMake(CGRectGetMaxX(previousButtonFrame) + self.interitemSpacing, previousButtonFrame.origin.y,
                ...
                for (UIButton *prevButton in processedButtons.reverseObjectEnumerator) {
                    if (contentWidth - spaceUsed < self.moreButton.frame.size.width + self.interitemSpacing) {
                        ...
                        spaceUsed -= prevButton.frame.size.width + self.interitemSpacing;
                        self.moreButton.frame = CGRectMake(prevButton.frame.origin.x + self.interitemSpacing, prevButton.frame.origin.y,
                                                           self.moreButton.frame.size.width, self.moreButton.frame.size.height);
                ...
                for (UIButton *leftButtons in self.mutableTagButtons) {
                    if (![processedButtons containsObject:leftButtons]) {
                        leftButtons.hidden = YES;
                    }
                }
                break;
            } else {
